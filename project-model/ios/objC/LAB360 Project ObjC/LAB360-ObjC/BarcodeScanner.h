...
typedef NS_ENUM(NSUInteger, RSPCamera) {
    RSPCameraBack,
    RSPCameraFront
};
 ...
 */
typedef NS_ENUM(NSUInteger, RSPTorchMode) {
    RSPTorchModeOff,
    RSPTorchModeOn,
};
...
/**
 *  The currently set camera. See RSPCamera for options.
 *
 *  @sa setCamera:error:
 */
@property (nonatomic, assign, readonly) RSPCamera camera;
 ...
 */
@property (nonatomic, assign) RSPTorchMode torchMode;
 ...
 */
@property (nonatomic, copy) void (^resultBlock)(NSArray<AVMetadataMachineReadableCodeObject *> *codes, UIImage *interestRectSnapshot);
 ...
 *
 *  @return An instance of BarcodeScanner
 */
 ...
 *  @sa setCamera:error:
 */
- (void)setCamera:(RSPCamera)camera NS_DEPRECATED_IOS(2.0, 2.0, "Use setCamera:error: instead");

/**
 *  Sets the camera. This operation may fail, e.g., when the device
 *  does not have the specified camera.
 *
 *  @param camera The camera to use, see RSPCamera for options.
