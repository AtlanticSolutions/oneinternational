...
static NSString *kErrorDomain = @"BarcodeScannerError";
...
@interface BarcodeScanner () <AVCaptureMetadataOutputObjectsDelegate, AVCapturePhotoCaptureDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
...
/*
 ...
 @abstract
 The capture device output for capturing metadata.
 */
...
- (instancetype)init {
    NSAssert(NO, @"BarcodeScanner init is not supported. Please use initWithPreviewView: \
             or initWithMetadataObjectTypes:previewView: to instantiate a BarcodeScanner");
    return nil;
}
...
- (instancetype)initWithMetadataObjectTypes:(NSArray<NSString *> *)metaDataObjectTypes previewView:(UIView *)previewView {
    ...
    NSAssert(metaDataObjectTypes.count > 0,
             @"Must initialize BarcodeScanner with at least one metaDataObjectTypes value.");
    ...
    if (self) {
        ...
        NSAssert(!([metaDataObjectTypes indexOfObject:AVMetadataObjectTypeFace] != NSNotFound),
                 @"The type %@ is not supported by BarcodeScanner.", AVMetadataObjectTypeFace);
...
+ (BOOL)hasCamera:(RSPCamera)camera {
...
+ (RSPCamera)oppositeCameraOf:(RSPCamera)camera {
    switch (camera) {
        case RSPCameraBack:
            return RSPCameraFront;
            
        case RSPCameraFront:
            return RSPCameraBack;
    }
...
- (BOOL)startScanningWithResultBlock:(void (^)(NSArray<AVMetadataMachineReadableCodeObject *> *codes, UIImage *interestRectSnapshot))resultBlock error:(NSError **)error {
...
- (void)stopScanning {
    ...
    // Turn the torch off
    self.torchMode = RSPTorchModeOff;
...
- (BOOL)hasOppositeCamera {
    RSPCamera otherCamera = [[self class] oppositeCameraOf:self.camera];
...
//    dispatch_queue_t queue = dispatch_queue_create("barcode_video_output_queue", NULL);
//    [self.videoOutput setSampleBufferDelegate:self queue:queue];
//
//    NSDictionary *pixelBufferOptions = [NSDictionary dictionaryWithObjectsAndKeys: [NSNumber numberWithUnsignedInt:kCVPixelFormatType_32BGRA], (id)kCVPixelBufferPixelFormatTypeKey, nil];
//    self.videoOutput.videoSettings = pixelBufferOptions;
//    self.videoOutput.alwaysDiscardsLateVideoFrames = YES;
//    [newSession addOutput:self.videoOutput];
...
- (AVCaptureDevice *)newCaptureDeviceWithCamera:(RSPCamera)camera {
...
+ (AVCaptureDevicePosition)devicePositionForCamera:(RSPCamera)camera {
    switch (camera) {
        case RSPCameraFront:
...
- (void)setupSessionQueue {
    ...
    self.privateSessionQueue = dispatch_queue_create("com.mikebuss.BarcodeScanner.captureSession", DISPATCH_QUEUE_SERIAL);
}
...
- (void)setTorchMode:(RSPTorchMode)torchMode {
    [self setTorchMode:torchMode error:nil];
}

- (BOOL)setTorchMode:(RSPTorchMode)torchMode error:(NSError **)error {
...
- (void)toggleTorch {
    switch (self.torchMode) {
        ...
        case RSPTorchModeOff:
            self.torchMode = RSPTorchModeOn;
            break;
    }
}

- (BOOL)updateForTorchMode:(RSPTorchMode)preferredTorchMode error:(NSError **)error {
    AVCaptureDevice *backCamera = [AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeVideo];
    AVCaptureTorchMode avTorchMode = [self avTorchModeForRSPTorchMode:preferredTorchMode];
...
- (AVCaptureTorchMode)avTorchModeForRSPTorchMode:(RSPTorchMode)torchMode {
    switch (torchMode) {
        case RSPTorchModeOn:
...
//    self.imageFromVideoOutput = [CIImage imageWithCVPixelBuffer:pixelBuffer];
//}

#pragma mark - Setters

- (void)setCamera:(RSPCamera)camera {
    [self setCamera:camera error:nil];
}

- (BOOL)setCamera:(RSPCamera)camera error:(NSError **)error {
    if (camera == _camera) {
        return YES;
    }
...
- (void)setScanRect:(CGRect)scanRect {
    NSAssert(!CGRectIsEmpty(scanRect), @"Unable to set an empty rectangle as the scanRect of BarcodeScanner");
