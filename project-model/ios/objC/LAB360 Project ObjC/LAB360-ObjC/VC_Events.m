...
@synthesize eventsDayList, isLoaded;
...
- (void)viewDidLoad {
    ...
    //Listas
    AppD.eventsList = [NSMutableArray new];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (!isLoaded){
        ...
        [self setupLayout:NSLocalizedString(@"SCREEN_TITLE_EVENTS", @"")];
        ...
        [connectionManager getAllEventsAvailableForMasterEventID:AppD.masterEventID withCompletionHandler:^(NSArray *response, NSError *error) {
            ...
            if (error){
                ...
                if (response){
                    ...
                    AppD.eventsList = [[NSMutableArray alloc] initWithArray:[tempResult sortedArrayUsingDescriptors:sortDescriptors]];
                    ...
                    if ([connectionManager isConnectionActive])
                    {
                        [connectionManager getEventsForUser:AppD.loggedUser.userID andMasterEventID:AppD.masterEventID withCompletionHandler:^(NSDictionary *response, NSError *error) {
                            ...
                            else
                            {
                                ...
                                for (Event* evento in AppD.eventsList) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == tvEventsList){
        ...
        Event *actualEvent = [AppD.eventsList objectAtIndex:indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView == tvEventsList){
        ...
        vcEventDesc.event = [((Event*)[AppD.eventsList objectAtIndex:indexPath.row]) copyObject];
        //
...
//    }
    ...
    for (Event *event in AppD.eventsList){
