...
@interface DocCameraView () <AVCaptureVideoDataOutputSampleBufferDelegate> {
...
- (void)captureImageWithCompletionHander:(void(^)(UIImage* image))completionHandler {
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
         ...
         if (weakSelf.isBorderDetectionEnabled) {
             ...
             enhancedImage  = [enhancedImage cropBordersWihtMargin:20.0f]; //40.0
...
- (CIDetector*)detector {
    ...
    switch (self.detectorType) {
        ...
        case DocScannerDetectorTypePerformance: return detectorPerf;
            break;
        case DocScannerDetectorTypeAccuracy:
        default: break;
    }
    return detectorHigh;
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    switch (self.cameraViewType) {
        case DocScannerViewTypeBlackAndWhite:        image = [image filteredImageUsingEnhanceFilter];
            break;
        case DocScannerViewTypeNormal:               image = [image filteredImageUsingContrastFilter];
            break;
        case DocScannerViewTypeUltraContrast:        image = [image filteredImageUsingUltraContrastWithGradient:self.gradient ];
            break;
        default:
            break;
    }
    ...
    if (self.isBorderDetectionEnabled) {
        ...
        if (_borderDetectLastRectangleFeature) {
            ...
            if ([self.delegate respondsToSelector:@selector(didDetectRectangle:withConfidence:)]) {
                ...
                if (confidence > 99 && [self.delegate respondsToSelector:@selector(didGainFullDetectionConfidence:)] && self.didNotifyFullConfidence == NO) {
