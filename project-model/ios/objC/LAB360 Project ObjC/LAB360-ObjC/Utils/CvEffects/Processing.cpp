    ...
    for (int i = 0; i < src.rows; i++)
        for (int j = 0; j < src.cols; j++)
        {
            ...
            if (alpha_value != 0)
            {
                ...
                dst.at<Vec4b>(i, j)[0] = (weight) * src.at<Vec4b>(i, j)[0] + (1 - weight) *  dst.at<Vec4b>(i, j)[0];
                dst.at<Vec4b>(i, j)[1] = (weight) * src.at<Vec4b>(i, j)[1] + (1 - weight) *  dst.at<Vec4b>(i, j)[1];
                dst.at<Vec4b>(i, j)[2] = (weight) * src.at<Vec4b>(i, j)[2] + (1 - weight) *  dst.at<Vec4b>(i, j)[2];
            }
        }
