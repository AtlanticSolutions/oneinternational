...
- (instancetype)initWithLicenseKey:(NSString *)licenseKey dataSetFile:(NSString *)path {
    if(self = [super init]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didReceiveDidEnterBackgroundNotification:) name:UIApplicationDidEnterBackgroundNotification object:nil];
    }
    return self;
}
...
- (Vuforia::DataSet *)loadObjectTrackerDataSet:(NSString*)dataFile
{
    ...
    if (NULL == objectTracker) {
        ...
        if (NULL != dataSet) {
            ...
            //NOTE:ericogt >> a versão abaixo é um modelo para carregar o arquivo de outro diretório
            //Ainda não é funcional pois nenhum tipo diferente de 'STORAGE_APPRESOURCE' funcionou =(
//            NSString *expandedPath = [[dataFile stringByExpandingTildeInPath] stringByStandardizingPath];
//            const char *cpath = [expandedPath cStringUsingEncoding:NSUTF8StringEncoding];
//            if (!dataSet->load(cpath, Vuforia::STORAGE_APP)) {
//                NSLog(@"ERROR: failed to load data set");
//                objectTracker->destroyDataSet(dataSet);
//                dataSet = NULL;
//            }
...
//    }
    ...
    _projectionMatrix = Vuforia::Tool::getProjectionGL(cameraCalibration, 0.05f, 5000.0f); //0.05f, 5000.0f);
